...
module.exports = {
  name: "messageCreate",
  execute(message) {
    const prefix = StateManager.guildPrefixCache.get(message.guildId);
    ...
    for (const command of commands.entries()) {
      ...
      textCommand = textCommand.match(new RegExp(`${escapeRegExp(prefix)}[a-zA-Z1-9\\-]+\\b`));
      ...
      if (prefix + command[0] === textCommand[0]) {
        ...
        command[1].execute({ type: commandTypes.TEXT, args: args, message: message });
      }
    }
  },
};
